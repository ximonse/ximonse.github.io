...
async function runAiSorting() {
    ...
    try {
        const payload = {
            model: 'gpt-4o', // Uppgraderat från gpt-4o-mini för bättre kvalitet
            temperature: 0.2,
...
async function askClaudeAboutNotes(query) {
    ...
    const MAX_CARDS = 150; // Analyze up to 150 most relevant cards (increased for better context)
    ...
    // Collect notes as structured JSON (more efficient for AI to parse)
    const cardsData = cardsToSend.map(node => ({
        id: node.id(),
        title: node.data('title') || '',
        text: node.data('text') || '',
        tags: node.data('tags') || [],
        hidden_tags: node.data('hidden_tags') || [],
        color: node.data('cardColor') || 'none',
        source: node.data('export_source') || 'manual',
        position: { x: Math.round(node.position().x), y: Math.round(node.position().y) }
    }));

    // Create readable summary for AI
    const notesContext = JSON.stringify(cardsData, null, 2);

    // Create enhanced prompt with better context
    let prompt = `Du är en EXPERT på visuell kunskapsorganisation. Du förstår hur människor tänker, strukturerar information och hittar samband.

🎯 DIN SPECIALITET:
Du är MYCKET bättre än generiska AI:er på att:
- Identifiera implicita teman och mönster
- Förstå vad användaren faktiskt vill uppnå (inte bara vad de säger)
- Skapa intuitiva visuella strukturer som "känns rätt"
- Föreslå smarta arrangemang användaren inte tänkt på
...
- arrange_clusters: Dela i kategorier/teman. Systemet skapar automatiskt högar med färgade etiketter som INTE överlappar.

➡️ SAMBAND & FLÖDEN:
- create_arrows: Rita sambandspilar mellan kort (orsak→verkan, steg 1→steg 2, referens→källa)
- create_annotation: Skapa stora rubrik-etiketter/markeringar

📏 STRUKTURERADE LAYOUT:
- arrange_grid: Rutnät (bra för jämförelser, kategorier)
- arrange_vertical_column: Vertikal kolumn (V-kommando, kort i rad uppifrån-ner)
- arrange_horizontal_row: Horisontell rad (H-kommando, kort i rad vänster-höger)
...
🔍 FILTRERING:
- filter_and_show: Visa bara vissa kort (fokusera på delmängd)

⚠️ KRITISK REGEL - ÖVERLAPPNING:
BARA arrange_organic_scatter (Q) och arrange_neat_stack (S) får överlappa kort!
ALLA andra verktyg (clusters, grid, column, row, circle, timeline) MÅSTE ha proper spacing mellan kort.
Systemet hanterar detta automatiskt, men välj RÄTT verktyg:
- Vill du överlappning? → Använd Q eller S
- Vill du separation? → Använd alla andra verktyg

📊 EXEMPEL PÅ SMART BETEENDE:

Användare: "organisera mina todo-kort"
...
🚀 VIKTIGT:
- Om uppdraget är tydligt: GÖR det direkt, förklara kort efteråt
- Om uppdraget är vagt: GE 2-3 konkreta alternativ baserat på vad du ser i korten
- Var PROAKTIV: Om du ser uppenbara mönster, påpeka dem
- Var PRAKTISK: Föreslå inte teoretiska lösningar, gör konkreta förslag baserat på DESSA kort

`;

    if (totalCards > MAX_CARDS) {
        prompt += `📊 KONTEXT: Totalt ${totalCards} kort på brädan. Du ser de ${cardsToSend.length} mest relevanta för denna fråga (filtrerats smart baserat på söktermer).

`;
    } else {
        prompt += `📊 KONTEXT: Du ser ALLA ${totalCards} kort på brädan.

`;
    }

    prompt += `🗂️ KORT-DATA (strukturerad JSON för enkel analys):
\`\`\`json
${notesContext}
\`\`\`
...
💬 SVARA PÅ SVENSKA:
- Tydligt uppdrag → Agera direkt med verktyg, förklara kort
- Vagt uppdrag → Analysera korten först, ge 2-3 konkreta förslag
- Oklart → Fråga smart (ge exempel på vad du kan göra)`;
    ...
    const tools = [
        ...
        {
            name: "arrange_vertical_column",
            description: "Arrange cards in single vertical column (V command). Cards aligned at top with spacing.",
            input_schema: {
                type: "object",
                properties: {
                    card_ids: {
                        type: "array",
                        items: { type: "string" },
                        description: "Card IDs to arrange in column"
                    }
                },
                required: ["card_ids"]
            }
        ...
        {
            name: "arrange_horizontal_row",
            description: "Arrange cards in horizontal row (H command). Cards aligned at top with spacing.",
            input_schema: {
                type: "object",
                properties: {
                    card_ids: {
                        type: "array",
                        items: { type: "string" },
                        description: "Card IDs to arrange in row"
                    }
                },
                required: ["card_ids"]
            }
        ...
        {
            name: "arrange_organic_scatter",
            description: "Arrange cards in organic scattered pile (Q command). Like messy pile on desk - cards slightly overlapping. Perfect for brainstorm mode.",
            input_schema: {
                type: "object",
                properties: {
                    card_ids: {
                        type: "array",
                        items: { type: "string" },
                        description: "Card IDs to scatter"
                    }
                },
                required: ["card_ids"]
            }
        ...
        {
            name: "arrange_neat_stack",
            description: "Arrange cards in neat aligned stack (S command). All cards perfectly aligned. Perfect for archive or unified group.",
            input_schema: {
                type: "object",
                properties: {
                    card_ids: {
    ...
    const response = await fetch('https://api.anthropic.com/v1/messages', {
        ...
        body: JSON.stringify({
            ...
            max_tokens: 8192,
            system: `Du är en EXPERT på visuell kunskapsorganisation med djup förståelse för hur människor strukturerar information.

KÄRNKOMPETENSER:
- Identifiera implicita mönster och samband i anteckningar
- Förstå användarens faktiska intention bakom frågor
- Skapa intuitiva visuella strukturer
- Vara proaktiv med smarta förslag

ARBETSSÄTT:
1. Analysera situationen först (vad vill användaren uppnå?)
2. Identifiera naturliga kategorier/teman i korten
3. Välj rätt verktyg för uppgiften
4. Agera smart: Ge konkreta alternativ istället för vaga frågor

PRINCIPER:
- Tydligt uppdrag → Agera direkt, förklara kort
- Vagt uppdrag → Ge 2-3 konkreta förslag baserade på kort-analys
- Var proaktiv: Påpeka uppenbara mönster
- Var praktisk: Basera förslag på DESSA kort, inte teoretiska möjligheter

ABSOLUT REGEL OM ÖVERLAPPNING:
- BARA Q (organic_scatter) och S (neat_stack) får överlappa kort
- ALLA andra verktyg (clusters, grids, columns, rows, etc.) MÅSTE ha separation mellan kort
- Om användaren vill "högar" eller "grupperingar" → använd arrange_clusters (NO overlap!)
- Om användaren vill "organisk brainstorm" → använd arrange_organic_scatter (overlap OK)
- Om användaren vill "arkiv/samla" → använd arrange_neat_stack (overlap OK)

Du är lika smart som de mest avancerade AI-assistenterna, men specialiserad på visuell organisation.`,
            tools: tools,
            messages: messages
        })
    });
...
async function executeAITool(toolName, input) {
    ...
    switch (toolName) {
        ...
        case 'create_arrows':
            ...
            break;
        case 'arrange_vertical_column':
            await aiArrangeVerticalColumn(input);
            break;
        case 'arrange_horizontal_row':
            await aiArrangeHorizontalRow(input);
            break;
        case 'arrange_organic_scatter':
            await aiArrangeOrganicScatter(input);
            break;
        case 'arrange_neat_stack':
            await aiArrangeNeatStack(input);
            break;
        default:
...
async function aiArrangeClusters(input) {
    ...
    // Card and spacing constants - IMPORTANT: Prevent overlap between clusters!
    const CARD_SPACING_X = 250;  // Horizontal space between cards within cluster
    const CARD_SPACING_Y = 200;  // Vertical space between cards within cluster
    const CLUSTER_MARGIN = 800;  // LARGE space between clusters to prevent ANY overlap
...
// AI Tool: Arrange cards in vertical column (V command)
async function aiArrangeVerticalColumn(input) {
    const { card_ids } = input;

    console.log(`📏 Arranging ${card_ids.length} cards in vertical column`);
    ...
    // Use existing column arrangement function
    arrangeSelectedInColumn();

    // Show status
    const statusDiv = document.getElementById('selectionInfo');
    if (statusDiv) {
        statusDiv.textContent = `AI arrangerade ${card_ids.length} kort i kolumn`;
        statusDiv.classList.add('visible');
        setTimeout(() => statusDiv.classList.remove('visible'), 3000);
    }
}

// AI Tool: Arrange cards in horizontal row (H command)
async function aiArrangeHorizontalRow(input) {
    const { card_ids } = input;

    console.log(`↔️ Arranging ${card_ids.length} cards in horizontal row`);
    ...
    // Use H command logic - horizontal alignment
    arrangeSelectedHorizontally();

    // Show status
    const statusDiv = document.getElementById('selectionInfo');
    if (statusDiv) {
        statusDiv.textContent = `AI arrangerade ${card_ids.length} kort horisontellt`;
        statusDiv.classList.add('visible');
        setTimeout(() => statusDiv.classList.remove('visible'), 3000);
    }
}

// AI Tool: Arrange cards in organic scatter (Q command)
async function aiArrangeOrganicScatter(input) {
    const { card_ids } = input;

    console.log(`🎲 Scattering ${card_ids.length} cards organically`);
    ...
    // Use Q command logic - organic scatter
    const selectedNodes = cy.$('node:selected');
    if (selectedNodes.length > 0) {
        const bounds = selectedNodes.boundingBox();
    ...
    const statusDiv = document.getElementById('selectionInfo');
    if (statusDiv) {
        statusDiv.textContent = `AI skapade organisk skräphög med ${card_ids.length} kort`;
        statusDiv.classList.add('visible');
        setTimeout(() => statusDiv.classList.remove('visible'), 3000);
    }
...
// AI Tool: Arrange cards in neat stack (S command)
async function aiArrangeNeatStack(input) {
    const { card_ids } = input;

    console.log(`📚 Stacking ${card_ids.length} cards neatly`);
    ...
    // Use S command logic - neat stack
    const selectedNodes = cy.$('node:selected');
    if (selectedNodes.length > 0) {
        const firstNode = selectedNodes[0];
        const stackX = firstNode.position().x;
        const stackY = firstNode.position().y;
    ...
    const statusDiv = document.getElementById('selectionInfo');
    if (statusDiv) {
        statusDiv.textContent = `AI stacked ${card_ids.length} kort prydligt`;
        statusDiv.classList.add('visible');
        setTimeout(() => statusDiv.classList.remove('visible'), 3000);
    }
...
function updateLinkBadges() {
    ...
    cy.nodes().forEach(node => {
        ...
        // Scale badge size proportionally to card size (8% of width, min 15px, max 30px)
        const badgeSize = Math.max(15, Math.min(30, width * 0.08));
        const fontSize = Math.max(10, badgeSize * 0.6);

        // Position inside top-right corner
        const leftPos = pos.x + width/2 - badgeSize * 1.5;
        const topPos = pos.y - height/2 + badgeSize * 0.5;
...
function showKeyboardShortcutsDialog() {
        ...
        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 25px;">
            ...
            <div class="shortcut-category">
                ...
                <div class="shortcut-list">
                    ...
                    <div class="shortcut-item"><kbd data-action="Q">Q</kbd><span>Runt kluster</span></div>
                    <div class="shortcut-item"><kbd data-action="QQ">QQ</kbd><span>Tight kluster</span></div>
                    <div class="shortcut-item"><kbd data-action="S">S</kbd><span>Stack (hög)</span></div>
                </div>
            </div>
            ...
            <div class="shortcut-category">
                ...
                <div class="shortcut-list">
                    <div class="shortcut-item"><kbd data-action="C">C</kbd><span>Kopiera markerade</span></div>
                    <div class="shortcut-item"><kbd data-action="Delete">Delete</kbd><span>Radera markerade</span></div>
                </div>
            </div>
            ...
            <div class="shortcut-category">
                ...
                <div class="shortcut-list">
                    ...
                    <div class="shortcut-item"><kbd data-action="Ctrl+F">Ctrl+F</kbd><span>Fokusera sökning</span></div>
                    <div class="shortcut-item"><kbd data-action="Escape">Escape</kbd><span>Avmarkera alla</span></div>
                </div>
            </div>
            ...
            <div class="shortcut-category">
                ...
                <div class="shortcut-list">
                    ...
                    <div class="shortcut-item"><kbd data-action="Ctrl+E">Ctrl+E</kbd><span>Exportera markerade</span></div>
                </div>
            </div>
            ...
            <div class="shortcut-category">
                ...
                <div class="shortcut-list">
                    <div class="shortcut-item"><kbd data-action="annotation">⬜→⭕</kbd><span>Öppna ritmeny</span></div>
                    <div class="shortcut-item" style="font-size: 12px; color: #999; padding-left: 62px;">
                        <span>Figurer: Rektangel, Cirkel, Triangel, Diamant, Stjärna, Hexagon</span>
                    </div>
                    <div class="shortcut-item" style="font-size: 12px; color: #999; padding-left: 62px;">
                        <span>Text: Små (S), Medium (M), Stor (L)</span>
                    </div>
                    <div class="shortcut-item" style="font-size: 12px; color: #999; padding-left: 62px;">
                        <span>Kopplingar: Pil (→), Linje (➖)</span>
                    </div>
                </div>
            </div>
            ...
            <div class="shortcut-category">
                ...
                <div class="shortcut-list">
                    <div class="shortcut-item"><kbd data-action="Shift+T">Shift+T</kbd><span>Toggle toolbar</span></div>
                    <div class="shortcut-item"><kbd data-action="M">M</kbd><span>Multi-import dialog</span></div>
                </div>
            </div>
        </div>
    ...
    kbdElements.forEach(kbd => {
        kbd.addEventListener('click', function() {
            ...
            switch(action) {
                // Arrangement
                case 'H': arrangeHorizontal(); break;
                case 'V': arrangeVertical(); break;
                case 'G+V': arrangeGridVertical(); break;
                case 'G+H': arrangeGridHorizontal(); break;
                case 'G+T': arrangeGridTight(); break;
                case 'Q': arrangeCluster(); break;
                case 'QQ': arrangeTightCluster(); break;
                case 'S': arrangeStack(); break;

                // Färg
                case '0': case '1': case '2': case '3': case '4': case '5': case '6':
                    applyColorToSelected(parseInt(action)); break;
                case 'T': showColorPicker(); break;
                ...
                // Ritverktyg
                case 'annotation': toggleAnnotationToolbar(); break;
